// components/Profile/WatchedIssuesList.js
'use client'
...
export default function WatchedIssuesList({ profileId }) {
    ...
    useEffect(() => {
        async function fetchWatched() {
            try {
                const token = getToken()
                if (!token) throw new Error('No token found to load watched issues')
                ...
                const res = await fetch(
                    `https://it22d-backend.onrender.com/api/profiles/${profileId}/watched-issues/`,
                    {
                        cache: 'no-store',
                        headers: {
                            Authorization: token,
                ...
                if (!res.ok) throw new Error('Error loading watched issues')
                const data = await res.json()
                console.log('Datos watched recibidos:', data) // Para debug
                setIssues(data)
            } catch (err) {
                console.error(err)
    ...
    const mapIssueData = (issue) => {
        console.log('Watched issue original:', issue) // Para debug
    ...
    if (loading) return <div className={styles.loadingMessage}>Loading Watched Issues…</div>
    if (error) return <div className={styles.errorMessage}>Error: {error}</div>
    if (!issues.length) return <p className={styles.emptyMessage}>You are not observing any issues.</p>
        ...
        <section className={styles.issueTable}>
            ...
            <ul className={styles.issuesList}>
                {issues.map((issue) => {
                    ...
                    console.log('Watched issue mapeado:', mappedIssue) // Para debug
